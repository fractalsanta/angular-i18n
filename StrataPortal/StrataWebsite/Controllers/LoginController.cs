...
namespace Rockend.iStrata.StrataWebsite.Controllers
{
    ...
    public class LoginController : BaseController
    {
        ...
        public ActionResult Login(int? aid)
        {
            ...
            {
                int appKey = 0;
                if (int.TryParse(SessionData.ApplicationKey, out appKey) || int.TryParse(AppKeyHelper.GetApplicationKeyFromUrl(), out appKey))
                {
                    aid = appKey;
                }              
            }
            ...
            return Redirect(string.Concat("/", aid.HasValue ? aid.Value.ToString() : string.Empty, "/", "Login", "/", "Logon")); // "Login", new { aid = aid });
        }
        ...
        public ActionResult Logon(LogonModel model)
        {
            ...
            else
            {
                ...
                if (SessionData != null && SessionData.ApplicationKey != null)
                {
            ...
            try
            {
                ...
                //Get the version and tack it onto the UserSession so we can use it in other places
                if (base.AgencyApplication != null)
                {
                    userSession.StrataVersionNumber = base.AgencyApplication.Version;
                }
                ...
                switch (role)
                {
                    ...
                    case Role.ExecutiveMember:
                        return RedirectToAction("Main", "CommitteeMember", new RouteValueDictionary(new { applicationKey = SessionData.ApplicationKey }));
                }
            }
            catch (Exception ex)
            {
        ...
        private bool CheckFileSmartStatus()
        {
            ...
            {
                ServiceAgencyApplication fileSmartService = CommunicatorData.LoadAgentServicesByServiceKey(AgencyApplication.ApplicationKey.Value, fileSmartServiceKey);
        ...
        public ActionResult FindPassword(string username, string userType)
        {
            ...
            try
            {
                ...
                SendEmail(new List<string> { email }, body, "STRATA Portal Details");
        ...
        public ActionResult FindUsername(string email, string userType)
        {
            ...
            try
            {
                ...
                body.AppendLine();
                SendEmail(new List<string> { email }, body.ToString(), "STRATA Portal Username");
