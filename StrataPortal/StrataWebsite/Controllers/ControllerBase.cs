...
namespace Rockend.iStrata.StrataWebsite.Controllers
{
    ...
    public class BaseController : Controller
    {
        ...
        protected AgencyApplication AgencyApplication
        {
        ...
        {
            // log some extra details to help identify why this occurred
            try
            {
                ...
                if (Request != null)
                {
                    info.AppendLine(string.Format("\tRawUrl: {0}", Request.RawUrl ?? ""));
                    info.AppendLine(string.Format("\tHttpMethod: {0}", Request.HttpMethod));
                    if(Request.UrlReferrer != null)
                        info.AppendLine(string.Format("\tUrlReferrer: {0}", Request.UrlReferrer.PathAndQuery));
                    info.AppendLine(string.Format("\tUserHostAddress: {0}", Request.UserHostAddress ?? "null"));
                }
                else
                {
                    info.Append("null request");
                }
                Logger.Warning("HandleUnknownAction: {0}{1}", actionName, info);
            }
            catch (Exception ex)
            {
                Logger.Error(ex);
        ...
        protected virtual void SetViewBagData()
        {
            ...
            if (UserSession != null)
            {
                if (UserSession.StrataVersionNumber < 1050) //Addiotional settings are only available from Strata v10.5
                {
                    ViewBag.StrataShowReports = true;
                ...
                else
                {
                    ...
                    {
                        ViewBag.StrataShowMeetings = AgentContentStrata.ShowMeetingsPageOwner;
                        ViewBag.StrataShowReports = AgentContentStrata.ShowReportsPageOwner;
                    }
                    if (UserSession.Role == StrataCommon.Role.ExecutiveMember)
                    {
                        ViewBag.StrataShowMeetings = AgentContentStrata.ShowMeetingsPageCommitteeMember;
                        ViewBag.StrataShowReports = AgentContentStrata.ShowReportsPageCommitteeMember;
                    }
                }
                //Old code below
                //ViewBag.StrataShowMeetings = AgentContentStrata.AgentContent.StrataShowMeetings;
            ...
            {
                ViewBag.StrataShowMeetings = false;
                ViewBag.StrataShowReports = false;
            }
            ...
            if (UserSession != null)
            {
                ...
                ViewBag.Title = UserSession.Role == StrataCommon.Role.ExecutiveMember ? "Committee" : "Owner";
            ...
            ViewBag.BannerUrl = string.Format("https://rockendstoragesydney.blob.core.windows.net/portals/{0}/images/{1}/Banner.jpg", env, SessionData.ApplicationKey);
            ViewBag.FooterBannerUrl = string.Format("https://rockendstoragesydney.blob.core.windows.net/portals/{0}/images/{1}/FooterBanner.jpg", env, SessionData.ApplicationKey);
            ViewBag.IPhoneIconUrl = string.Format("https://rockendstoragesydney.blob.core.windows.net/portals/{0}/images/{1}/IPhoneIcon.jpg", env, SessionData.ApplicationKey);
        }
        ...
        protected void InitializeSessionData()
        {
            ...
            if (result)
            {
                ...
                ServiceAgencyApplication strataPortalService = CommunicatorData.LoadAgentServicesByServiceKey(key, portalServiceKey);
