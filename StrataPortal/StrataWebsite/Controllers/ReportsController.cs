...
namespace Rockend.iStrata.StrataWebsite.Controllers
{
    ...
    public class ReportsController : BaseController
    {
        ...
        {
            if (UserSession == null || UserSession.LotNames == null || UserSession.LotNames[index] == null || !UserCanViewReports())
            {
...
//        [FileDownload()]  do not use unless you wire up the spinner properly, otherwise it creates a cookie which doesn't get cleaned up...because client side for this report currently does not use the cookie (my guess)
...
//        [FileDownload()]  do not use unless you wire up the spinner properly, otherwise it creates a cookie which doesn't get cleaned up...because client side for this report currently does not use the cookie (my guess)
        ...
        public ActionResult rptCurrentOwnerAccountRun(int planId, int ownerId)
        {
            ...
            if (UserSession == null || UserSession.LotOwners == null)
            {
                return RedirectToAction("Logout", "Account");
            }
        ...
        private ActionResult HandleReportResponse(ReportResponse response)
        {
            if (response == null || response.pdfFile == null || response.pdfFile.Length == 0)
            {
                ...
                {
                    url = string.Concat("https://", Request.UrlReferrer.Authority, Request.UrlReferrer.LocalPath);
        ...
        private bool UserCanViewReports()
        {
            if (UserSession != null)
            {
                if(UserSession.StrataVersionNumber < 1050)
                {
                    return true; //In older versions everyone can view reports
                }
                
                if (UserSession.Role == StrataCommon.Role.Owner)
                {
                    return AgentContentStrata.ShowReportsPageOwner;
                }
                if (UserSession.Role == StrataCommon.Role.ExecutiveMember)
                {
                    return AgentContentStrata.ShowReportsPageCommitteeMember;
                }
            }
            return false;
        }
