    ...
    public static class ReportHelper
    {
        ...
        public static bool CanAccessReport(this UserSession session, string name, int corpOwnerID)
        {
            ...
            if (session != null)
            {
                ...
                canAccess = session.CanAccessReport(report, corpOwnerID);
            }          
        ...
        public static bool CanAccessReport(this UserSession session, WebAccessReports report, int planId)
        {
            ...
            if (report.IsAllowed.ToBoolean())
            {
                if (perm != null)
                {
                    if (session.Role == Role.ExecutiveMember && (perm.AllowedExecutive || perm.AllowedOwner))
                    {
                        return true;
                    }
