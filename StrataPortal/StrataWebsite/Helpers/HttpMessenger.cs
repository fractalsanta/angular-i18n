    ...
    public class HttpMessenger : IMessenger
    {
        ...
        protected object Send(object request, string actionName = "")
        {
            ...
            catch (Exception ex)
            {
                ...
                var tempEx = new Exception(string.Format("Action: {1}{0}Request: {2}{0}AppKey: {3}{0}", Environment.NewLine, actionName, request.ToString(), ApplicationKey), ex);
                throw new StrataWebException("An error occurred contacting the agency", tempEx);
            }
        }
            ...
            where TResponse : class
        {
            ...
            if (result is ErrorResponse)
            {
                ...
                var ex = new Exception(string.Format("Action: {1}{0}Request: {2}{0}AppKey: {3}{0}Error: {4}{0}", Environment.NewLine, actionName, request.ToString(), ApplicationKey, errorResponse.ErrorMessage), errorResponse.Exception);
                throw new StrataWebException(errorResponse.ErrorMessage, ex);
            }
        ...
        public MeetingAgendaReponse GetMeetingAgenda(int lotId, int meetingRegisterId)
        {
            var request = new MeetingAgendaRequest()
            {
                LotID = lotId,
                MeetingRegisterId = meetingRegisterId
            };

            return Send<MeetingAgendaRequest, MeetingAgendaReponse>(request, "GetMeetingAgenda");
        }

        public VoteResponse SyncVotesToStrata(MeetingAgendaReponse meetingAgenda)
        {            
            return Send<string, VoteResponse>(SerializeObject<MeetingAgendaReponse>(meetingAgenda), "SyncVoteResults");
        }
