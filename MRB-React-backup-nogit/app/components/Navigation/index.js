...
function Navigation({ toggleDrawer, isDrawerOpen, navigateToFeedback, navigateToCreateFeedback, sendItemsInPostQueue }) {
  ...
  return (
      ...
      <Drawer open={isDrawerOpen} docked={false} containerStyle={isDrawerOpen ? {width: 'calc(100% - 56px)', maxWidth: "400px"} : ''}  overlayClassName={"drawerOverlay"} onRequestChange={toggleDrawer} >
          ...
          <List style={{padding:"8px"}}>
            <RaisedButton label="Change Store" buttonStyle={{height:"56px"}} fullWidth={true} labelPosition="before" icon={<NavigationArrowForward />} iconStyle={{color:"red"}} labelStyle={{color: "#fff", fontWeight: 600, textTransform: "none"}} backgroundColor={"rgb(3,155,229)"} />
          </List>
      </Drawer>
      </div>
  );
