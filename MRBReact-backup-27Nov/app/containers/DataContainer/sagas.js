...
function* fetchAllData() {
  ...
  const stores =
    typeof userFactsStores === "string" &&
    userFactsStores.toLowerCase() === "all"
      ? storesUnfiltered
...
function* sendItemsInPostQueue() {
  ...
  for (let i = 0; i <= postQueue.length - 1; i++) {
    try {
      yield call(
        dataService[postQueue[i].postFunction],
        activeNamespace,
        postQueue[i].postObject
      );
      yield put(removeItemFromPostQueue(postQueue[i].uuid));
      // const newDoc = {
      //   store_id: "59d51a7b8047e1172240603e",
      //   business_day: "2017-09-11",
      //   content_url: "none",
      //   status: "complete",
      //   mrblist: "Daily Page - Manager's Summary"
      // };
      if (postQueue[i].successFunction) {
        debugger;
        yield put(actions[postQueue[i].successFunction](postQueue[i].successObject));
      }
      debugger;
    } catch (e) {}
  }
