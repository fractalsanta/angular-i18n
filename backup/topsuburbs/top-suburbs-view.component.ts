...
export class TopSuburbsViewComponent implements OnInit, OnDestroy {
  chartData: ICustomChartData<ISuburb>;
  ...
  ngOnInit() {
    this.filterSub = this.store.select(fromStore.getFilterData).subscribe((filterState: IFilter) => {
      ...
      this.mapChartData(this.filter, this.topSuburbsData);
    });
  ...
  mapChartData(filterState: IFilter | null, topSuburbsData: ISuburb[] | null): void {
    if (topSuburbsData === null) {
      this.chartData = { data: [] };
      return;
    }

    this.suburbs = _.chain(topSuburbsData)
      ...
      .value();
    this.chartData = this.mapToCustomChart(this.suburbs);
    console.log(this.chartData);
  }

  mapToCustomChart(topSuburbsData: ISuburb[] | null): IMultiChartDictionary {
    if (topSuburbsData === null) {
      return {};
    } else {
